...
- **La base del CI no tiene `seed.ts`**: solo migraciones y el rol de aplicación. Los catálogos que siembra el seed (`roles`, `crops`, tipos de canal…) **no están**, así que una prueba que necesite uno lo da de alta ella misma (`upsert`), como hacen `campaigns-service.spec.ts` y `notebook-catalog.spec.ts`. Aprendido a base de un fallo (2026-09-23): un miembro con alcance por finca choca con `members_role_code_fkey`, en local pasa porque la base de desarrollo sí está sembrada, y el rojo aparece solo en el pipeline — que además se lleva por delante los dos despliegues. Para reproducir la condición del CI: base nueva, `prisma migrate deploy` con el usuario dueño, `infra/docker/postgres-init/01-app-role.sql`, y la suite con `iot_platform_app`.
