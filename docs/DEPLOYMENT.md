...
| DNS | **Cloudflare** (plan gratuito) | DNS gratuito. Sin proxy desde el 2026-09-16 (sección 6, `BACKLOG.md` #51): se pierde su mitigación básica de DoS y su CDN a cambio de no depender de IPs compartidas que se bloquean en España |
...
- **DNS**: gestionado en Cloudflare (plan gratuito), **sin proxiar ningún registro** (gris/DNS-only) desde el 2026-09-16: `api`, `app` y `mqtt` apuntan directamente al servidor. Hasta esa fecha `api`/`app` iban proxiados (naranja), pero las IPs compartidas de Cloudflare que les tocaban las bloquean los grandes operadores españoles en días de fútbol por orden de LaLiga, y la plataforma dejaba de abrir para sus clientes (`BACKLOG.md` #51). El TLS lo da Caddy en el origen (Let's Encrypt, renovación automática) y el de MQTT, EMQX.
...
- **Firewall** (Hetzner Cloud Firewall, a nivel de VPS): entrante permitido solo en 80 (HTTP, exclusivamente para el reto ACME HTTP-01 de Let's Encrypt y la redirección a HTTPS — nunca sirve contenido de negocio), 443 (HTTPS/WSS), 8883 (MQTTS) y 22 (SSH, restringido a IPs de administración conocidas — nunca abierto a cualquier IP). Todo lo demás, denegado por defecto. El puerto 80 se añadió en vivo (2026-08-04, primer despliegue real de staging): con `api`/`app` proxiados por Cloudflare, el reto TLS-ALPN-01 de Caddy falla siempre (Cloudflare termina el TLS en su borde), y sin el 80 abierto el reto HTTP-01 de repuesto tampoco podía llegar al origen (Cloudflare devolvía 522). En staging, además, **5555/udp** para el puente de la estación Dragino WSC2-N (2026-09-16, `public_udp_ports` en `modules/compute`, `BACKLOG.md` #44): abierto a cualquier IP porque la SIM de la estación no tiene IP fija; el propio puente descarta lo que no venga con el IMEI de la estación. El puente corre en su propio compose ([`infra/docker/docker-compose.puente.yml`](../infra/docker/docker-compose.puente.yml)), fuera de `deploy.sh`, conectado a la red interna de la plataforma para publicar en EMQX sin exponer el 1883.
...
- [x] Compilar la web en WebAssembly (2026-09-16, `BACKLOG.md` #50): `flutter build web --wasm --release --dart-define=API_BASE_URL=https://staging-api.jmvsoluciones.com` (sin `/v1`) y copiar `build/web/` a `/opt/iot-platform/web/`. Caddy ya sirve `main.dart.wasm` como `application/wasm` y comprimido. La versión anterior en JS queda en `/opt/iot-platform/web.anterior-js` por si hay que volver atrás. (El despliegue era manual hasta el 2026-09-22; ver la tarea siguiente.)
- [x] **Al sustituir la carpeta de la web, reiniciar Caddy** (2026-09-21, descubierto desplegando el #56). El procedimiento manual sube la compilación a `/opt/iot-platform/web.nueva` y renombra: `mv web web.anterior-<fecha>` + `mv web.nueva web`. Con eso, en disco está la versión nueva, pero **Caddy sigue sirviendo la vieja**: el `bind mount` de `./web` se resuelve al crear el contenedor y se queda apuntando al directorio anterior (al inodo, no a la ruta), así que un renombrado no le llega. Se ve en las cabeceras: `Last-Modified` y `Content-Length` de la versión anterior mientras `md5sum` en el servidor da la nueva. Solución: `docker restart iot-platform-caddy-1` (un par de segundos de corte). **Comprobación de que el despliegue ha llegado**, comparando lo servido con lo compilado — no basta con un 200:
  ```
  curl -s https://staging-app.jmvsoluciones.com/main.dart.wasm | md5sum
  md5sum apps/mobile/build/web/main.dart.wasm
  ```
  Ojo con comparar solo `flutter_bootstrap.js`: puede salir idéntico entre dos compilaciones y dar un falso «ya está desplegado».
- [x] **Automatizar el despliegue de la web** (2026-09-22, `BACKLOG.md` #61). Dos trabajos nuevos en `ci-cd.yml`: `web-build` compila una sola vez en un runner de GitHub (`flutter build web --wasm --release --dart-define=API_BASE_URL=https://staging-api.jmvsoluciones.com`) y sube el resultado como artefacto `web-staging`; `deploy-web-staging` lo publica desde el **runner autoalojado de la propia VPS** (`runs-on: [self-hosted, staging]`), igual que `deploy-staging`, así que no hace falta SSH desde ninguna IP. Tres motivos, todos vividos:
  1. El procedimiento manual dependía de que la IP de quien despliega pudiera entrar por el puerto 22, y el firewall solo admite la IP de administración conocida (misma razón por la que existe el runner autoalojado). El 2026-09-22 esa conexión dejó de responder y la web de la fase 4 de campañas se quedó sin subir.
  2. El reinicio de Caddy tras el cambio de carpeta va **dentro** del trabajo (`docker restart` del contenedor que localiza `docker ps --filter name=caddy`), en vez de depender de que alguien se acuerde.
  3. La limpieza de copias antiguas ordena **por fecha** (`ls -dt web.anterior-* | tail -n +3`), no por nombre. Ordenar por nombre (`ls -d … | sort | head -n -2`) borró en un despliegue manual la copia recién hecha, porque los sufijos de fecha (`web.anterior-2026…`) ordenan antes que los antiguos de texto (`web.anterior-js`).
...
| 2026-09-16 | `api`/`app` sin proxy de Cloudflare, apuntando directamente al servidor (`BACKLOG.md` #51) | Mantener el proxy y aceptar las caídas en días de fútbol (descartado: 17 días bloqueada en dos meses, en todos los grandes operadores); IP dedicada de Cloudflare o BYOIP (plan Enterprise, fuera de escala) |
