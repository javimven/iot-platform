...
- [`infra/docker/docker-compose.deploy.yml`](../infra/docker/docker-compose.deploy.yml) + [`Caddyfile`](../infra/docker/Caddyfile) + [`deploy.sh`](../infra/docker/deploy.sh) + [`.env.example`](../infra/docker/.env.example) (2026-07-30): despliegue real distinto del `docker-compose.yml` de desarrollo — `api` con 2 réplicas nombradas (despliegue sin interrupciones, sección 8), `worker`/`ingestion`/`redis`/`emqx` autoalojados, Caddy como reverse proxy + TLS automático + servidor de estáticos para la app Flutter web, `otel-collector` reenviando a Grafana Cloud (nuevo [`infra/otel/otel-collector-prod.yaml`](../infra/otel/otel-collector-prod.yaml), OBSERVABILITY.md §2). `deploy.sh` aplica primero los `GRANT` que Terraform no puede (las tablas no existen todavía cuando se aplica `infra/terraform/modules/database`, nuevo [`infra/docker/postgres-init/02-grant-app-role-managed.sql`](../infra/docker/postgres-init/02-grant-app-role-managed.sql)), migra el esquema, y despliega `api_1`/`api_2` uno detrás del otro comprobando `/health`. Validado en local sin ninguna cuenta externa: `docker compose config`, `caddy validate`/`fmt`, `shellcheck` — todo limpio.
  ...
  - **El commit con este fix pasó limpio en local pero falló Trivy en el primer run real de CI — segundo bug crítico, `BACKLOG.md` #23**: `npm prune --omit=dev` (etapa `build`) no había quitado `eslint`/`jest`/`typescript`/`@nestjs/cli` de la imagen publicada en CI (362 MB de `node_modules` frente a 180 MB en una build local idéntica, incluso con `--no-cache`) — sin explicación completa encontrada para la discrepancia entre entornos. Corregido eliminando la dependencia en `prune`: nueva etapa `prod-deps` con un `npm ci --omit=dev` dedicado. Verificado de nuevo: 180 MB, Trivy en 0 hallazgos, mismo login real repetido con éxito.
...
- [x] Escribir el Docker Compose/Caddyfile/script de despliegue real (`infra/docker/docker-compose.deploy.yml`/`Caddyfile`/`deploy.sh`, 2026-07-30) — corrigió de paso `BACKLOG.md` #22 (bug crítico: la imagen `runtime` no podía conectar con Postgres) y #23 (encontrado en el primer run real de CI tras #22: `npm prune` dejaba devDependencies enteras dentro de la imagen).
...
- [ ] Copiar `docker-compose.deploy.yml`/`Caddyfile`/`deploy.sh` a la VPS real y ejecutar un primer despliegue de verdad — hoy solo validado en local (`docker compose config`, `caddy validate`, `shellcheck`, y un login real de principio a fin dentro del contenedor), nunca contra la VPS/DNS/TLS reales.
- [ ] Extender `ci-cd.yml` para que los pasos 9/12 (hoy `echo`) invoquen `deploy.sh` de verdad por SSH, una vez exista una VPS real donde probarlo — necesita los secretos de conexión (host/clave SSH) como GitHub Actions Environment secrets.
