...
- [x] Compilar la web en WebAssembly (2026-09-16, `BACKLOG.md` #50): `flutter build web --wasm --release --dart-define=API_BASE_URL=https://staging-api.jmvsoluciones.com` (sin `/v1`) y copiar `build/web/` a `/opt/iot-platform/web/`. Caddy ya sirve `main.dart.wasm` como `application/wasm` y comprimido. El despliegue de la web sigue siendo manual; la versión anterior en JS queda en `/opt/iot-platform/web.anterior-js` por si hay que volver atrás.
