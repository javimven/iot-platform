...
- **Fotos y documentos por `multipart/form-data`** (fase 6), en el campo `file`, con un tope de 15 MB. **El formato lo decide el contenido, no el `Content-Type` del cliente**: se miran los primeros bytes y se admiten JPG, PNG, WEBP, HEIC y PDF; un ejecutable anunciado como imagen no entra. Se adjuntan a la campaña (`/campaigns/{id}/documents`) o a una actividad (`/campaigns/{id}/activities/{activityId}/documents`), y `GET /campaigns/{id}/documents` los devuelve **todos de una vez** —los de la campaña y los de sus actividades— para que la línea de tiempo marque cuáles llevan foto sin una petición por actividad.
- **Nada sale del bucket sin firmar**: cada documento viaja con una `url` firmada que caduca a los 15 minutos (`urlExpiresAt`). No se guarda ni se comparte: se vuelve a pedir la lista. Es el mismo `StorageService` y el mismo bucket privado que el satélite.
- **`GET /campaigns/{id}/export` devuelve el fichero, no un enlace** (fase 7): `format=pdf|json|csv` y, para el PDF, `layout=informe|cue`. Se genera en el momento; guardarlo en el bucket dejaría copias que envejecen mientras el cuaderno sigue vivo. El nombre viaja en `Content-Disposition` con `filename*` en UTF-8, porque lleva el del cultivo y sus acentos. **Una campaña cerrada se exporta desde su instantánea de cierre** ([ADR-0012](ADR/0012-campanas-unidades-de-cultivo-y-actividades.md)): cambiar hoy el titular de la finca no cambia el cuaderno cerrado de ayer.
- **Los tres formatos salen del mismo modelo** (`schemaVersion`, ADR-0014). Si cada uno recogiera sus datos por su cuenta, acabarían diciendo cosas distintas del mismo cuaderno. El CSV va con punto y coma, BOM y coma decimal, que es lo que Excel en español abre de un doble clic, y un tratamiento con dos productos da dos filas.
- **Un fichero se guarda una sola vez** por organización (SHA-256): subir la misma factura a tres tratamientos ocupa una. Y **quitarla de un sitio no la borra de los demás**: se quita el vínculo, y solo cuando no queda ninguno se da de baja el documento. El objeto se conserva, que es lo que permite auditar qué se subió.
