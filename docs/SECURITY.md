...
- **Bug crítico real encontrado en vivo (2026-07-29): `GET /members` filtraba el hash Argon2 de la contraseña de cada miembro.** `MembersService#listForOrganization` usaba `include: { user: true }` (Prisma) y devolvía el objeto crudo directamente como respuesta HTTP — sin ningún DTO/`ClassSerializerInterceptor` que excluyera campos, `user.passwordHash` viajaba tal cual al cliente. Alcance: cualquier `org_admin` (único rol con `members.read`) veía el hash de todos los miembros de **su propia** organización al listar miembros — RLS ya impedía ver los de otras organizaciones, pero un hash de contraseña nunca debe llegar a ningún cliente, ni siquiera al de su propio equipo (superficie de ataque de fuerza bruta offline si ese hash se filtra después por otra vía — captura de red, logs, DevTools). Encontrado al construir la pantalla de Miembros en Flutter y comparar la respuesta real del endpoint contra el esquema `Member` documentado en `OPENAPI.yaml` — la respuesta real ni siquiera coincidía con lo documentado (`user` anidado en vez de `email`/`fullName` planos), y al mirar qué había dentro de ese `user` anidado apareció el hash. Corregido cambiando a `include: { user: { select: { email: true, fullName: true } } } }` (Prisma nunca llega a pedirle `passwordHash` a la base de datos, no es solo un filtrado posterior) y aplanando la respuesta al formato documentado. Verificado en vivo: `GET /members` con un token real de `org_admin` ya no incluye `passwordHash` ni el objeto `user` anidado. Se revisó el resto del código en busca del mismo patrón (`include: { user: true }`) — el único otro uso (`notification-dispatch.service.ts`) es interno, para leer el email al enviar una notificación, nunca se serializa en una respuesta HTTP. Al verificar en vivo también `POST /members` (invitar) y `PATCH /members/:id` (cambiar rol/estado) contra el cliente Flutter real, se encontró que ninguno de los dos devolvía `email`/`fullName` en absoluto (no llegaban a filtrar el hash, pero tampoco cumplían el esquema documentado — habría roto el parseo del modelo `Member` en Flutter con un error de tipo, no silencioso). Corregidos igual: `invite()` reutiliza el `targetUser` ya cargado en la misma transacción (sin consulta extra), `updateRoleOrStatus()` usa el mismo `select` que `listForOrganization`.
