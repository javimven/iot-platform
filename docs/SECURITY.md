...
- **Riesgo pendiente registrado, no bloqueante todavía** (2026-07-28): tras `--omit=dev` quedan 11 hallazgos reales (3 high, 8 moderate), todos resueltos solo con un salto de versión mayor de NestJS (v10 → v11: `@nestjs/core`, `@nestjs/common`, `@nestjs/config`, `@nestjs/platform-express`, `@nestjs/cli`):
  - `lodash` (alto, inyección de código/prototype pollution) vía `@nestjs/config` — expuesto solo en el arranque al leer configuración, no ante input de un atacante.
  - `multer` (alto, DoS) vía `@nestjs/platform-express` — **no alcanzable hoy**: el backend no define ninguna ruta con `FileInterceptor`/`MulterModule` (verificado, no hay subida de ficheros implementada).
  - `qs`/`body-parser` (moderado, DoS) vía `@nestjs/platform-express` — sí alcanzable, ya que procesa el body/query de cada petición entrante.
  - Se acepta como riesgo conocido y documentado por ahora (no crítico, sin exposición directa salvo `qs`, que es DoS no compromiso de datos); pendiente de una subida dedicada de NestJS v10 → v11 con su propia ronda de revalidación (build, los 46 tests unitarios, los tests de integración RLS, y arranque real de los 3 procesos api/ingestion/worker contra infraestructura real) antes de intentarlo dentro del ciclo normal de trabajo, no como parche apresurado.
  - `nodemailer` (antes en esta lista, CVEs de inyección SMTP/CRLF/SSRF) — **ya corregido**: subido de `6.10.1` a `9.0.3` (única versión sin estos CVEs), verificado con un envío real a través de Mailpit, sin cambios de código necesarios (superficie de API usada — `createTransport`/`sendMail` con host/port/auth — se mantuvo estable).
