...
2. **Umbral por timeout**, a nivel de dato (cubre dispositivos sin conexión propia, y también gateways cuya conexión sigue técnicamente viva pero han dejado de enviar datos): se marca offline si no se recibe una lectura válida en **2,5 × el intervalo de heartbeat esperado** (Etapa 5, `heartbeat_interval_seconds`) — con un intervalo de 30 min, offline a los ~75 min; con 60 min, a los ~150 min. El multiplicador de 2,5 tolera **un** ciclo de reporte perdido por una pérdida temporal de conectividad (Etapa 1) sin disparar una falsa alerta, pero no dos. Si un gateway no fija su propio `heartbeat_interval_seconds`, el valor por defecto es 144 min (offline a las ~6h) — subido desde los 15 min iniciales (2026-08-10, feedback en vivo: ese valor original marcaba offline con demasiada frecuencia estaciones reales que solo tardaban algo más entre lecturas).
