...
- La creación/mantenimiento de particiones mensuales de `telemetry` **no** es una migración de esquema — es un job operativo programado que crea las particiones futuras con antelación. Desde el 2026-09-21 existe: la función `crear_particiones_telemetry(meses)` (migración `0006_particiones_telemetry`, `SECURITY DEFINER` porque el rol de la aplicación no crea tablas) y `TelemetryPartitionsService` en el worker, que la llama al arrancar y cada doce horas manteniendo tres meses por delante (BACKLOG.md #45).
