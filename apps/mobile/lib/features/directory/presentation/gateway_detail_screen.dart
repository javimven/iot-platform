...
import 'directory_screen.dart' show showConfirmDialog, showGatewayCredentialDialog;
...
class GatewayDetailScreen extends ConsumerWidget {
        ...
        actions: [
          if (canManage) ...[
            IconButton(
              ...
              onPressed: () => _showEditGatewayDialog(context, ref, gateway.value),
            ),
            IconButton(
  ...
  Future<void> _showEditGatewayDialog(BuildContext context, WidgetRef ref, Gateway? gateway) async {
            ...
            onPressed: () async {
              ...
              try {
                await ref
                    ...
                    .updateGateway(gatewayId, name: nameController.text.trim());
    ...
    if (saved == true) {
      ref.invalidate(gatewayProvider(gatewayId));
    }
  }
  ...
  Future<void> _confirmDisable(BuildContext context, WidgetRef ref) async {
    ...
    if (!confirmed) return;
    try {
      await ref.read(directoryApiProvider).disableGateway(gatewayId);
      ref.invalidate(gatewayProvider(gatewayId));
    } catch (error) {
      if (context.mounted) {
